...
6. `apps/api/prisma/migrations/20260515000000_add_pending_chain_tx/migration.sql`  
7. FISCO BCOS 官方文档与 WeBASE-Front 接口文档  
8. MySQL 8.0 官方文档  
9. 《GB/T 8567-2006 计算机软件文档编制规范》
...
```mermaid
graph LR
  M[管理员] --> P4[节点状态/交易溯源/统计分析/待上链队列]
  P4 --> C[(FISCO BCOS)]
  C --> P4
  P4 --> D6[(ChainTransaction)]
  D6 --> P4
  P4 --> D7[(PendingChainTx)]
  D7 --> P4
  P4 --> O4[链状态与交易查询结果]
  O4 --> M
```
...
| 10 | 待上链队列信息 | 链操作失败暂存与重试 | bizType + bizId + contractName + funcName + funcParam + status + retryCount + nextRetryAt |
...
| 8 | status | 稿件/任务/队列状态 | ENUM/String | 见业务状态集 | 驱动流程分支 |
...
| 17 | retryCount | 已重试次数 | INT | >=0 | 与 maxRetries 比较决定是否继续重试 |
| 18 | nextRetryAt | 下次重试时间 | DATETIME(3) | 当前时间之后 | 与 status 组合为检索条件 |
| 19 | confirmedTxHash | 确认交易哈希 | VARCHAR(191) | 0x 开头哈希 | 重试成功后写入，关联 `ChainTransaction.txHash` |
...
7. `PendingChainTx`（待上链交易队列）
- 关键属性：id, bizType, bizId, contractName, funcName, status, retryCount, nextRetryAt, maxRetries
...
7) 图3-7 PendingChainTx 实体属性图

```mermaid
graph TD
  PT[PendingChainTx] --> PT1[id PK]
  PT --> PT2[bizType]
  PT --> PT3[bizId]
  PT --> PT4[contractName]
  PT --> PT5[contractAddress]
  PT --> PT6[funcName]
  PT --> PT7[funcParam JSON]
  PT --> PT8[maxRetries]
  PT --> PT9[retryCount]
  PT --> PT10[nextRetryAt]
  PT --> PT11[status]
  PT --> PT12[lastError]
  PT --> PT13[confirmedTxHash]
  PT --> PT14[confirmedBlockHeight]
  PT --> PT15[createdAt]
  PT --> PT16[updatedAt]
```
...
- `PendingChainTx` 通过 `bizType + bizId` 与业务对象逻辑关联，主要关联稿件 ID，记录链上操作失败后的排队重试信息。
...
- `PendingChainTx` 通过 `bizType + bizId` 逻辑关联业务对象（主要为稿件），在链不可达时暂存待重试交易，重试成功后回写 `confirmedTxHash` 和 `confirmedBlockHeight`。
...
| 7 | 待上链队列表 | `PendingChainTx` | 链操作失败后的排队重试记录 |
...
作用：当链操作（版权存证、审稿提交、裁定上链）因网络或节点异常失败时，将失败交易暂存至本表，由 ChainRetryService 定时扫描并自动重试，实现链上操作的最终一致性。
...
| 物理表名 | `PendingChainTx` |
| 主键 | `id` |
| 业务主键 | 无（通过 `bizType + bizId` 逻辑关联业务对象） |
| 索引 | `PendingChainTx_status_nextRetryAt_idx`（status + nextRetryAt 复合索引），`PendingChainTx_bizType_bizId_idx`（bizType + bizId 复合索引） |

字段列表：

| 序号 | 中文名称 | 列名 | 数据类型 | 非空 | 外键表（字段） |
|------|----------|------|----------|------|----------------|
| 1 | 队列ID | id | VARCHAR(191) | 是 | 无 |
| 2 | 业务类型 | bizType | VARCHAR(191) | 是 | 无 |
| 3 | 业务ID | bizId | VARCHAR(191) | 是 | 无 |
| 4 | 合约名称 | contractName | VARCHAR(191) | 是 | 无 |
| 5 | 合约地址 | contractAddress | VARCHAR(191) | 是 | 无 |
| 6 | 函数名称 | funcName | VARCHAR(191) | 是 | 无 |
| 7 | 函数参数 | funcParam | JSON | 是 | 无 |
| 8 | 最大重试次数 | maxRetries | INT | 是 | 无 |
| 9 | 已重试次数 | retryCount | INT | 是 | 无 |
| 10 | 下次重试时间 | nextRetryAt | DATETIME(3) | 是 | 无 |
| 11 | 队列状态 | status | VARCHAR(191) | 是 | 无 |
| 12 | 最近错误 | lastError | VARCHAR(191) | 否 | 无 |
| 13 | 确认交易哈希 | confirmedTxHash | VARCHAR(191) | 否 | 无 |
| 14 | 确认区块高度 | confirmedBlockHeight | INT | 否 | 无 |
| 15 | 创建时间 | createdAt | DATETIME(3) | 是 | 无 |
| 16 | 更新时间 | updatedAt | DATETIME(3) | 是 | 无 |

状态说明：
- `PENDING`：等待重试（默认初始状态）
- `RETRYING`：正在重试中（防止并发重复执行）
- `CONFIRMED`：重试成功，已回写业务表
- `FAILED`：超过 maxRetries 上限，标记为最终失败

业务类型（bizType）说明：
- `COPYRIGHT_CERTIFY`：版权存证上链失败
- `REVIEW_SUBMIT`：审稿提交上链失败
- `ADJUDICATE`：裁定结果上链失败

重试策略：ChainRetryService 每 30 秒扫描 `status='PENDING' AND nextRetryAt <= now()` 的记录，采用指数退避策略（30s × 2^retryCount，上限 5 分钟），默认最多重试 5 次。重试成功后自动回写对应业务表（Paper、ReviewResult、ReviewTask）的 txHash/blockHeight，并补录 ChainTransaction 记录。
...
- `ADMIN`：用户管理、审稿分配、裁定、链查询、会议配置、待上链队列管理
...
本文档内容与以下代码现状保持一致：
- 数据模型：`apps/api/prisma/schema.prisma`（含 PendingChainTx）
- 迁移：`apps/api/prisma/migrations/*`（含 20260515000000_add_pending_chain_tx）
- 业务写库路径：`apps/api/src/*/*.service.ts`（含 ChainRetryService 排队重试逻辑）
